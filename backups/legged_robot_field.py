...
class LeggedRobotField(LeggedRobot):
    ...
    def compute_observations(self):
        ...
        for i in range(self.num_envs):  
            ...
            #             for dof_index, prop in enumerate(current_dof_props)
            #         }
            #         # print(i,current_dof_props_tensors)
            #         tmp_dof_index= dof_index - self.num_actions
            #         # Make sure limits are set
            #         if ((current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index]['effort']) and (current_dof_props_tensors[tmp_dof_index]['velocity'] == custom_dof_limit[tmp_dof_index]['velocity']) and (current_dof_props_tensors[tmp_dof_index]['lower'] == custom_dof_limit[tmp_dof_index]['lower']) and (current_dof_props_tensors[tmp_dof_index]['upper'] == custom_dof_limit[tmp_dof_index]['upper'])):
            #             for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #                 if tmp_dof_index in dof_indices:
            #                     self.obs_buf[i, -12 + tmp_dof_index] = 1  # 标记第 dof_index 位
            #                     self.obs_buf[i, -15:-12] = 1 # 表示已经损坏
            ...
            #         else:
            #             raise ValueError(f"DOF properties at index {tmp_dof_index} do not match custom limits, env is {i}. "
            #                      f"Current: {current_dof_props_tensors[tmp_dof_index]}, "
            #                      f"Custom: {custom_dof_limit[tmp_dof_index]}")

            # # 对于24-35分组，dof损坏且不加mask，obs保持原样
            
            # # 对于36-47分组，保持为Normal Setting
