...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        if not self.training:  # 如果是评估模式
            torch.backends.mha.set_fastpath_enabled(False)  # 禁用 fast path
        ...
        x = src
        if self.norm_first:
            x_tmp, self.attn_weight = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, is_causal=is_causal)
            x = x + x_tmp
            x = x + self._ff_block(self.norm2(x))
        else:
            x_tmp, self.attn_weight = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)
    ...
    def _sa_block(self, x: Tensor,
        ...
        attn_weight= self.self_attn(x, x, x,
                           attn_mask=attn_mask,
                           key_padding_mask=key_padding_mask,
                           need_weights=True, is_causal=is_causal)[1]
