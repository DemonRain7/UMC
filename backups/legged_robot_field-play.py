...
class LeggedRobotField(LeggedRobot):
    ...
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        # random.seed(10)
        self.threshold_malfunction_added_iter = 75
        print("malfunction_added_iter:", self.threshold_malfunction_added_iter)
    ...
    def _create_onboard_camera(self, env_handle, actor_handle, sensor_name):
        ...
        if hasattr(getattr(self.cfg.sensor, sensor_name), "horizontal_fov"):
            ...
            ) if isinstance(getattr(self.cfg.sensor, sensor_name).horizontal_fov, (tuple, list)) else getattr(
                self.cfg.sensor, sensor_name).horizontal_fov
    ...
    def check_termination(self):
        ...
        if getattr(self.cfg.termination, "check_obstacle_conditioned_threshold",
                   False) and self.check_BarrierTrack_terrain():
    ...
    def _get_terrain_curriculum_move(self, env_ids):
        if not (self.cfg.terrain.selected == "BarrierTrack" and self.cfg.terrain.BarrierTrack_kwargs[
            "virtual_terrain"] and hasattr(self, "body_sample_indices")):
        ...
        moved = distance > (
                    self.terrain.env_block_length * 1.5)  # 0.1 is the guess of robot touching the obstacle block.
    ...
    def _init_buffers(self):
        ...
        if getattr(self.cfg.domain_rand, "randomize_gravity_bias", False):
            ...
            self.gravity_bias[:, 0] *= self.cfg.domain_rand.gravity_bias_range["x"][1] - \
                                       self.cfg.domain_rand.gravity_bias_range["x"][0]
            self.gravity_bias[:, 0] += self.cfg.domain_rand.gravity_bias_range["x"][0]
            self.gravity_bias[:, 1] *= self.cfg.domain_rand.gravity_bias_range["y"][1] - \
                                       self.cfg.domain_rand.gravity_bias_range["y"][0]
            self.gravity_bias[:, 1] += self.cfg.domain_rand.gravity_bias_range["y"][0]
            self.gravity_bias[:, 2] *= self.cfg.domain_rand.gravity_bias_range["z"][1] - \
                                       self.cfg.domain_rand.gravity_bias_range["z"][0]
    ...
    def _reset_buffers(self, env_ids):
        ...
        if getattr(self.cfg.domain_rand, "randomize_gravity_bias", False):
            ...
            self.gravity_bias[env_ids, 0] *= self.cfg.domain_rand.gravity_bias_range["x"][1] - \
                                             self.cfg.domain_rand.gravity_bias_range["x"][0]
            self.gravity_bias[env_ids, 0] += self.cfg.domain_rand.gravity_bias_range["x"][0]
            self.gravity_bias[env_ids, 1] *= self.cfg.domain_rand.gravity_bias_range["y"][1] - \
                                             self.cfg.domain_rand.gravity_bias_range["y"][0]
            self.gravity_bias[env_ids, 1] += self.cfg.domain_rand.gravity_bias_range["y"][0]
            self.gravity_bias[env_ids, 2] *= self.cfg.domain_rand.gravity_bias_range["z"][1] - \
                                             self.cfg.domain_rand.gravity_bias_range["z"][0]
    ...
    def _init_volume_sample_points(self):
        ...
        self.body_sample_indices = torch.tensor(self.body_sample_indices,
                                                device=self.sim_device).flatten()  # n_envs * num_bodies
    ...
    def _get_mask_obs_obs(self, privileged=False):
        return torch.zeros((self.num_envs, 12), device=self.sim_device)
    ...
    def _get_obs_from_components(self, components: list, privileged=False):
        ...
        for k, v in obs_segments.items():
            if k == "proprioception":
                ...
                if k != "mask_obs" and k != "is_dof_limit":
                    obs.append(
    ...
    def compute_observations(self):
        ...
        if getattr(self.cfg.domain_rand, "randomize_gravity_bias",
                   False) and "proprioception" in self.cfg.env.obs_components:
        ...
        # print("actor obs_buf before", self.obs_buf)

        # envs_per_dof = max(1, self.num_envs // (self.num_actions + self.num_actions + self.num_actions + self.num_actions + self.num_actions))  # 每组的环境数量：no_doflimit + doflimit + 0obs_nomask + malfunc_nomask + noise_nomask + normal
        # envs_per_dof = max(1, self.num_envs // 5)
        envs_per_dof = max(1, self.num_envs // (self.num_actions+1))
        # 定义关节的映射关系，包含 indices 和 dof_indices
        joint_mapping_for_obs = {
            "FL_hip": ([12, 24, 36], [0]),  # FL_hip
            "FL_thigh": ([13, 25, 37], [1]),  # FL_thigh
            "FL_calf": ([14, 26, 38], [2]),  # FL_calf
            "FR_hip": ([15, 27, 39], [3]),  # FR_hip
            "FR_thigh": ([16, 28, 40], [4]),  # FR_thigh
            "FR_calf": ([17, 29, 41], [5]),  # FR_calf
            "RL_hip": ([18, 30, 42], [6]),  # RL_hip
            "RL_thigh": ([19, 31, 43], [7]),  # RL_thigh
            "RL_calf": ([20, 32, 44], [8]),  # RL_calf
            "RR_hip": ([21, 33, 45], [9]),  # RR_hip
            "RR_thigh": ([22, 34, 46], [10]),  # RR_thigh
            "RR_calf": ([23, 35, 47], [11]),  # RR_calf
        }
        ...
        # print("dof_differences",dof_differences)
        # critic obs
        ...
        if getattr(self.cfg.domain_rand, "randomize_privileged_gravity_bias", False) and "proprioception" in getattr(
                self.cfg.env, "privileged_obs_components", []):
            assert hasattr(self, "gravity_bias")
            proprioception_slice = get_obs_slice(self.privileged_obs_segments, "proprioception")
            # print(proprioception_slice)
            self.privileged_obs_buf[:,
            proprioception_slice[0].start + 6: proprioception_slice[0].start + 9] += self.gravity_bias

        # print("critic obs_buf before", self.privileged_obs_buf)
        # 计算并保存6组最大值和最小值
        # print("LegrobotField:",self.curr_iter)
        for i in range(self.num_envs):
            ...
            #     if self.curr_iter > self.cfg.obs.threshold_zerodof_mask:  # 当迭代次数达到阈值后，开始加入损坏
            ...
            #                         # 模拟损坏，obs 置0，设置检测标记
            #                         self.obs_buf[i, -12 + tmp_dof_index] = 1
            #                         for idx in indices:
            ...
            #     if self.curr_iter > self.cfg.obs.threshold_zeroNmakfuncdof_mask:  # 当迭代次数达到阈值后，开始加入损坏
            #         # 随机选择不超过3个 DOF
            #         # Determine which dof group this environment belongs to
            ...
            #             for tmp_dof_index in faulty_dof_indices:
            #                 # 检查当前DOF是否设置了限制
            #                 if (current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index]['effort']):
            #                     for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #                         if tmp_dof_index in dof_indices:
            #                             self.obs_buf[i, -12 + tmp_dof_index] = 1  # 标记第 dof_index 位
            #                             self.obs_buf[i, -15:-12] = 1  # 表示已经损坏
            ...
            # # 动态且随机选择不超过3个OBS损坏
            # if self.cfg.obs.dynamic_fault_obs and dof_index==2:
            #     if self.curr_iter > self.cfg.obs.threshold_zerodof_nomask:  # 当迭代次数达到阈值后，开始加入损坏
            #         if i in self.dof_limit_dict:  # 如果当前环境ID存在于字典中
            #             faulty_dof_indices = self.dof_limit_dict[i]
            #             # print(i,dof_index,faulty_dof_indices)
            #             for tmp_dof_index in faulty_dof_indices:
            ...
            # # 动态且随机选择不超过3个DOF损坏
            # if self.cfg.obs.dynamic_fault_obs and dof_index==3:
            #     if self.curr_iter > self.cfg.obs.threshold_zeroNmakfuncdof_nomask:  # 当迭代次数达到阈值后，开始加入损坏
            #         # 随机选择不超过3个 DOF
            #         # Determine which dof group this environment belongs to
            ...
            # # 动态且随机加huge-scale噪声
            # if self.cfg.obs.dynamic_fault_obs and dof_index==4:
            #     if self.curr_iter > self.cfg.obs.threshold_noise_nomask:  # 当迭代次数达到阈值后，开始加入损坏
            #         # print("BEFORE:",i, self.obs_buf[i,0:48])
            #         self.obs_buf[i,0:48] += (2 * torch.rand_like(self.obs_buf[i,0:48]) - 1) * self.noise_scale_vec[0:48] * 10
            #         # print("AFTER:",i,self.obs_buf[i,0:48])

            # # 最后一组不变

            # dof_index = self.num_actions + self.num_actions + self.num_actions + self.num_actions + self.num_actions - 1
            # # Determine which dof group this environment belongs to
            # if i < ((self.num_actions + self.num_actions + self.num_actions + self.num_actions + self.num_actions - 1) * envs_per_dof):
            #     dof_index = i // envs_per_dof
            #
            # # 对于0-11分组，置0+mask
            # if self.cfg.obs.dynamic_fault_obs:
            #     # dof_index在0-11的时候做obs的mask
            #     if 0 <= dof_index < self.num_actions:
            #         # 遍历关节映射字典，找到对应的dof_index
            #         for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #             if dof_index in dof_indices:
            #                 self.obs_buf[i, -12 + dof_index] = 1  # 标记第 dof_index 位
            #                 for idx in indices:
            #                     self.obs_buf[i, idx] = 0
            #                 break
            #
            # # 对于12-23分组，doflimit+置0+mask
            # if self.cfg.obs.dynamic_fault_obs:
            #     # Determine which dof group this environment belongs to
            #     if self.num_actions <= dof_index < self.num_actions * 2:
            #
            #         current_dof_props = self.gym.get_actor_dof_properties(self.envs[i], self.actor_handles[i])
            #         current_dof_props_tensors = {
            #             dof_index: {
            ...
            #             for dof_index, prop in enumerate(current_dof_props)
            #         }
            #         tmp_dof_index = dof_index - self.num_actions
            #         # Make sure limits are set
            #         if (current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index][
            #             'effort']):
            #             for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #                 if tmp_dof_index in dof_indices:
            #                     self.obs_buf[i, -12 + tmp_dof_index] = 1  # 标记第 dof_index 位
            #                     self.obs_buf[i, -15:-12] = 1  # 表示已经损坏
            #                     # self.privileged_obs_buf[i, -12 + tmp_dof_index] = 1
            #                     for idx in indices:
            #                         self.obs_buf[i, idx] = 0
            #                         # self.privileged_obs_buf[i, idx] = 0
            #                     break
            #
            #         else:
            #             raise ValueError(f"DOF properties at index {tmp_dof_index} do not match custom limits. "
            ...
            # # 对于24-35分组，置0不加mask
            # if self.cfg.obs.dynamic_fault_obs:
            #     if 2 * self.num_actions <= dof_index < 3 * self.num_actions:
            #         # 遍历关节映射字典，找到对应的dof_index
            #         tmp_dof_index = dof_index - 2 * self.num_actions
            #         for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #             if tmp_dof_index in dof_indices:
            #                 for idx in indices:
            #                     self.obs_buf[i, idx] = 0
            #                 break
            #
            # # 对于36-47分组，置0且dof损坏不加mask
            # if self.cfg.obs.dynamic_fault_obs:
            #     # Determine which dof group this environment belongs to
            #     if 3 * self.num_actions <= dof_index < self.num_actions * 4:
            #
            #         current_dof_props = self.gym.get_actor_dof_properties(self.envs[i], self.actor_handles[i])
            #         current_dof_props_tensors = {
            #             dof_index: {
            ...
            #             for dof_index, prop in enumerate(current_dof_props)
            #         }
            #         tmp_dof_index = dof_index - 3 * self.num_actions
            #         # Make sure limits are set
            #         if (current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index][
            #             'effort']):
            #             for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            ...
            # # 对于48-59分组，模拟大干扰
            # if self.cfg.obs.dynamic_fault_obs:
            #     if 4*self.num_actions <= dof_index < self.num_actions*5:
            #         # print("BEFORE:",i, self.obs_buf[i,0:48])
            #         self.obs_buf[i,0:48] += (2 * torch.rand_like(self.obs_buf[i,0:48]) - 1) * self.noise_scale_vec[0:48] * 10
            #         # print("AFTER:",i,self.obs_buf[i,0:48])
            ...
            # TEST 专用
            dof_index = self.num_actions
            # Determine which dof group this environment belongs to
            if i < ((self.num_actions) * envs_per_dof):
                dof_index = i // envs_per_dof
            ...
            #                     for idx in indices:
            #                         self.obs_buf[i, idx] = 0  # 将观测置为0，表示损坏
            #                     # faulty_count += 1  # 增加计数器
            #                     # if faulty_count >= 4:  # 当损坏的 DOF 达到4个时，退出循环
            #                     #     break
            #                     # 模拟损坏，obs 置0，设置检测标记
            #                     self.obs_buf[i, -12 + tmp_dof_index] = 1
            #                     break
            ...
            # }
            # # print(dof_index)
            # tmp_dof_index = dof_index
            # tmp_dof_props = self.gym.get_actor_dof_properties(self.envs[i], self.actor_handles[i])
            # # print(i,tmp_dof_props)
            # # Make sure limits are set
            # if (tmp_dof_index < 12):
            #     if (current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index]['effort'] and
            #             current_dof_props_tensors[tmp_dof_index]['velocity'] == custom_dof_limit[tmp_dof_index][
            #                 'velocity'] and current_dof_props_tensors[tmp_dof_index]['lower'] ==
            #             custom_dof_limit[tmp_dof_index]['lower'] and current_dof_props_tensors[tmp_dof_index][
            #                 'upper'] == custom_dof_limit[tmp_dof_index]['upper']):
            #         for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #             if tmp_dof_index in dof_indices:
            #                 self.obs_buf[i, -12 + tmp_dof_index] = 1  # 标记第 dof_index 位
            #                 self.obs_buf[i, -15:-12] = 1  # 表示已经损坏
            ...
            # # 6、1个dof被限制掉+过一会才坏
            # if self.curr_iter >= random_value:
            #     if self.curr_iter ==random_value and (dof_index < 12):
            #         current_dof_props_tensors = self.gym.get_actor_dof_properties(self.envs[i], self.actor_handles[i])
            #         # 保存env ID和随机选择的dof位置信息
            #         current_dof_props_tensors['effort'][dof_index] = self.cfg.asset.custom_dof_limit[dof_index]['effort']
            #         current_dof_props_tensors['velocity'][dof_index] = self.cfg.asset.custom_dof_limit[dof_index]['velocity']
            #         current_dof_props_tensors['lower'][dof_index] = self.cfg.asset.custom_dof_limit[dof_index]['lower']
            #         current_dof_props_tensors['upper'][dof_index] = self.cfg.asset.custom_dof_limit[dof_index]['upper']
            #         # 应用更新的 DOF 属性
            #         self.gym.set_actor_dof_properties(self.envs[i], self.actor_handles[i], current_dof_props_tensors)
            #
            #     # Make sure limits are set
            #     tmp_dof_index = dof_index
            #     current_dof_props_tensors = self.gym.get_actor_dof_properties(self.envs[i], self.actor_handles[i])
            #     if (tmp_dof_index < 12):
            #         if (current_dof_props_tensors[tmp_dof_index]['effort'] == custom_dof_limit[tmp_dof_index][
            #             'effort'] and current_dof_props_tensors[tmp_dof_index]['velocity'] ==
            #                 custom_dof_limit[tmp_dof_index]['velocity'] and
            #                 current_dof_props_tensors[tmp_dof_index]['lower'] == custom_dof_limit[tmp_dof_index][
            #                     'lower'] and current_dof_props_tensors[tmp_dof_index]['upper'] ==
            #                 custom_dof_limit[tmp_dof_index]['upper']):
            #             for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
            #                 if tmp_dof_index in dof_indices:
            #                     self.obs_buf[i, -12 + tmp_dof_index] = 1  # 标记第 dof_index 位
            #                     self.obs_buf[i, -15:-12] = 1  # 表示已经损坏
            ...
            #                     for idx in indices:
            #                         # self.obs_buf[i, idx] = 0  # 将观测置为0，表示损坏
            #                         self.obs_buf[i, idx] += 10
            #                     # faulty_count += 1  # 增加计数器
            #                     # if faulty_count >= 4:  # 当损坏的 DOF 达到3个时，退出循环
            #                     #     break
            #                     # 模拟损坏，obs 置0，设置检测标记
            #                     self.obs_buf[i, -12 + tmp_dof_index] = 1
            #                     break
            ...
            # 8.random dof 损坏且是随机在开始后的某一个时间段对你的dof进行锁死
            if self.curr_iter >= self.threshold_malfunction_added_iter:
                if self.curr_iter == self.threshold_malfunction_added_iter:
                    if i == 0:
                        print("Episode ", self.curr_iter, " reached, malfunctions added!")
                        # 获取机器人在malfunc后的初始位置(只需要第一次加入当前timestep下的initial_pos的obs即可)
                        root_states = self.gym.acquire_actor_root_state_tensor(self.sim)
                        root_states = gymtorch.wrap_tensor(root_states)
                        root_states = root_states.view(self.num_envs, -1, 13)[:, 0, :]  # (num_envs, 13)
                        self.initial_malfunction_positions = root_states[:, :3].clone()
                        print(self.initial_malfunction_positions)
                    if i in self.dof_limit_dict:  # 如果当前环境ID存在于字典中
                        faulty_dof_indices = self.dof_limit_dict[i]

                        # 获取 DOF 属性 tensor
                        current_dof_props_tensors = self.gym.get_actor_dof_properties(self.envs[i],
                                                                                      self.actor_handles[i])

                        # 获取 DOF 状态 tensor
                        dof_state_tensor = self.gym.acquire_dof_state_tensor(self.sim)
                        dof_state_tensor_torch = gymtorch.wrap_tensor(dof_state_tensor)

                        # 将 DOF 位置拆分出来
                        dof_pos_tensor = dof_state_tensor_torch[:, 0]  # 第一列是位置
                        dof_start_index = i * self.num_actions
                        # print(dof_pos_tensor.shape)

                        if i not in self.saved_dof_limits:
                            self.saved_dof_limits[i] = {}

                        # 然后你可以直接修改 tensor 里面的 DOF 属性
                        for tmp_dof_index in faulty_dof_indices:
                            current_dof_pos = dof_pos_tensor[dof_start_index+ tmp_dof_index].item()  # 获取当前 DOF 的位置
                            difference = dof_differences[tmp_dof_index]
                            # print("difference", difference)
                            # print("current_dof_pos before change", current_dof_pos)

                            # 例如在开始循环时或者第一次使用 saved_dof_limits 时：

                            # 计算 upper 和 lower 并保存
                            self.saved_dof_limits[i][tmp_dof_index] = {
                                "upper": current_dof_pos + difference,
                                "lower": current_dof_pos - difference
                            }
                            # print("saved_dof_limits[",i,"][",tmp_dof_index,"]", self.saved_dof_limits[i][tmp_dof_index])

                            # 直接访问和修改
                            current_dof_props_tensors['effort'][tmp_dof_index] = custom_dof_limit[tmp_dof_index]['effort']
                            # print(current_dof_props_tensors['effort'][tmp_dof_index])
                            # current_dof_props_tensors['velocity'][tmp_dof_index] = custom_dof_limit[tmp_dof_index]['velocity']
                            # print("upper before change:", current_dof_props_tensors['upper'][tmp_dof_index])
                            # print("lower before change:", current_dof_props_tensors['lower'][tmp_dof_index])

                            # current_dof_props_tensors['upper'][tmp_dof_index] = self.saved_dof_limits[i][tmp_dof_index][
                            #     'upper']
                            # current_dof_props_tensors['lower'][tmp_dof_index] = self.saved_dof_limits[i][tmp_dof_index][
                            #     'lower']

                            # print("upper after change:", current_dof_props_tensors['upper'][tmp_dof_index])
                            # print("lower after change:", current_dof_props_tensors['lower'][tmp_dof_index])
                            modified_dof_props = self._process_dof_props(current_dof_props_tensors, i)

                        # 使用同步方法将 DOF 属性更新回模拟中
                        self.gym.set_actor_dof_properties(self.envs[i], self.actor_handles[i],
                                                          current_dof_props_tensors)
                    else:
                        raise ValueError(f"DOF properties have no error for env{i}")
                    ...
                    for tmp_dof_index in faulty_dof_indices:
                        for dof_name, (indices, dof_indices) in joint_mapping_for_obs.items():
                            if tmp_dof_index in dof_indices:
                                ...
                                self.obs_buf[i, -12 + tmp_dof_index] = 0 # 标记第 dof_index 位
                                self.obs_buf[i, -15:-12] = 0 # 表示已经损坏
                                # for idx in indices:
                                #     self.obs_buf[i, idx] = 0
                                break

                else:
                    raise ValueError(f"DOF properties has no error for env{i}")

            # # 9.Normal Training
            # if self.curr_iter == self.threshold_malfunction_added_iter:
            #     if i == 0:
            #         print("Episode ", self.curr_iter, " reached, initial pos recorded!")
            #         # 获取机器人在malfunc后的初始位置(只需要第一次加入当前timestep下的initial_pos的obs即可)
            #         root_states = self.gym.acquire_actor_root_state_tensor(self.sim)
            #         root_states = gymtorch.wrap_tensor(root_states)
            #         root_states = root_states.view(self.num_envs, -1, 13)[:, 0, :]  # (num_envs, 13)
            ...
            # # 10、random obs坏掉
            # if self.curr_iter >= self.threshold_malfunction_added_iter:  # 当迭代次数达到阈值后，开始加入损坏
            #     if self.curr_iter == self.threshold_malfunction_added_iter:
            #         if i == 0:
            #             print("Episode ", self.curr_iter, " reached, malfunctions added!")
            #             # 获取机器人在malfunc后的初始位置(只需要第一次加入当前timestep下的initial_pos的obs即可)
        ...
        # print("actor obs_buf after", self.obs_buf.shape)
        # print("actor obs_buf after")
        # print(self.obs_buf.shape, self.obs_buf[:, -15:])
        # print("actor actions:", self.obs_buf[:, 36:48])
        # print("actor dofpos:", self.obs_buf[:, 12:24])
        # print("actor dofvels:", self.obs_buf[:, 24:36])

        # print("critic obs_buf after")
        # print(self.privileged_obs_buf.shape,self.privileged_obs_buf[:, -15:])
        # print("critic actions:", self.privileged_obs_buf[:, 36:48])
        # print("critic dofpos:", self.privileged_obs_buf[:, 12:24])
        # print("critic dofvels:", self.privileged_obs_buf[:, 24:36])
    ...
    def _write_forward_depth_noise(self, noise_vec):
        noise_vec[
        :] = self.cfg.noise.noise_scales.forward_depth * self.cfg.noise.noise_level * self.obs_scales.forward_depth
    ...
    def _write_robot_config_noise(self, noise_vec):
        if not hasattr(self.cfg.noise.noise_scales, "robot_config"):
            return
        noise_vec[
        :] = self.cfg.noise.noise_scales.robot_config * self.cfg.noise.noise_level * self.obs_scales.robot_config

    def _write_engaging_block_noise(self, noise_vec):
        if not hasattr(self.cfg.noise.noise_scales, "engaging_block"):
            return
        noise_vec[
        :] = self.cfg.noise.noise_scales.engaging_block * self.cfg.noise.noise_level * self.obs_scales.engaging_block

    def _write_sidewall_distance_noise(self, noise_vec):
        if not hasattr(self.cfg.noise.noise_scales, "sidewall_distance"):
            return
        noise_vec[
        :] = self.cfg.noise.noise_scales.sidewall_distance * self.cfg.noise.noise_level * self.obs_scales.sidewall_distance
    ...
    def _create_envs(self):
        ...
        self.front_hip_indices = torch.zeros(len(front_hip_names), dtype=torch.long, device=self.device,
                                             requires_grad=False)
        ...
        self.rear_hip_indices = torch.zeros(len(rear_hip_names), dtype=torch.long, device=self.device,
                                            requires_grad=False)
        ...
        if env_id == 0:
            if hasattr(self.cfg.control, "torque_limits"):
                if not isinstance(self.cfg.control.torque_limits, (tuple, list)):
                    self.torque_limits = torch.ones(self.num_dof, dtype=torch.float, device=self.device,
                                                    requires_grad=False)
                    self.torque_limits *= self.cfg.control.torque_limits
                else:
                    self.torque_limits = torch.tensor(self.cfg.control.torque_limits, dtype=torch.float,
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if hasattr(self, "robot_config_buffer"):
            ...
            self.robot_config_buffer[env_id, 5:5 + 12] = self.motor_strength[env_id] if hasattr(self,
                                                                                                "motor_strength") else 1.
        return props
    ...
    def get_obs_segment_from_components(self, components):
        ...
        if "engaging_block" in components:
            if not self.check_BarrierTrack_terrain():
                ...
                segments["engaging_block"] = get_terrain_cls("BarrierTrack").get_engaging_block_info_shape(
                    self.cfg.terrain)
        ...
        if "mask_obs" in components:
            segments["mask_obs"] = (12,)  # 添加 12 位追踪 mask
        # 添加 is_dof_limit 的处理
    ...
    def refresh_volume_sample_points(self):
        ...
        for body_idx, body_measure_name in enumerate(self.body_measure_name_order):
            ...
            self.velocity_sample_points[valid_velocity_mask,
            sample_points_start_idx: sample_points_start_idx + num_volume_points] = \
                (point_positions[valid_velocity_mask] - self.volume_sample_points[valid_velocity_mask,
                                                        sample_points_start_idx: sample_points_start_idx + num_volume_points]) / self.dt
            self.volume_sample_points[:,
            sample_points_start_idx: sample_points_start_idx + num_volume_points] = point_positions
    ...
    def draw_volume_sample_points(self):
        ...
        if self.cfg.terrain.selected == "BarrierTrack":
            penetration_mask = self.terrain.get_penetration_mask(self.volume_sample_points.view(-1, 3)).view(
                self.num_envs, -1)
    ...
    def _reward_penetrate_depth(self):
        ...
        self.refresh_volume_sample_points()
        penetration_depths = self.terrain.get_penetration_depths(self.volume_sample_points.view(-1, 3)).view(
            self.num_envs, -1)
    ...
    def _reward_penetrate_volume(self):
        if not self.check_BarrierTrack_terrain(): return torch.zeros_like(self.root_states[:, 0])
        self.refresh_volume_sample_points()
        penetration_mask = self.terrain.get_penetration_mask(self.volume_sample_points.view(-1, 3)).view(self.num_envs,
                                                                                                         -1)
    ...
    def _reward_tilt_cond(self):
        ...
        return_ = torch.where(tilting_mask, torch.clip(torch.abs(roll), 0, torch.pi / 2),
                              -torch.clip(torch.abs(roll), 0, torch.pi / 2))
        return return_
    ...
    def _reward_front_hip_pos(self):
        ...
        return torch.sum(
            torch.square(self.dof_pos[:, self.front_hip_indices] - self.default_dof_pos[:, self.front_hip_indices]),
            dim=1)

    def _reward_rear_hip_pos(self):
        """ Reward the robot to stop moving its rear hips """
        return torch.sum(
            torch.square(self.dof_pos[:, self.rear_hip_indices] - self.default_dof_pos[:, self.rear_hip_indices]),
            dim=1)
    ...
    def _reward_down_cond(self):
        ...
        return torch.exp(
            -pitch_err / self.cfg.rewards.tracking_sigma) * engaging_mask  # the higher positive factor, the more you want the robot to pitch down 0.2 rad
    ...
    def _reward_jump_x_vel_cond(self):
        ...
        engaging_mask = (engaging_obstacle_info[:, 1 + self.terrain.track_options_id_dict["jump"]] > 0) \
                        & (engaging_obstacle_info[:, 1 + self.terrain.max_track_options + 2] > 0.) \
                        & (engaging_obstacle_info[:,
                           0] > 0)  # engaging jump-up, not engaging jump-down, positive distance.
        roll, pitch, yaw = get_euler_xyz(self.root_states[:, 3:7])
        pi = torch.acos(torch.zeros(1)).item() * 2  # which is 3.1415927410125732
        pitch[pitch > pi] -= pi * 2  # to range (-pi, pi)
        pitch[pitch < -pi] += pi * 2  # to range (-pi, pi)
    ...
    def _reward_leap_bonous_cond(self):
        ...
        engaging_mask = (engaging_obstacle_info[:, 1 + self.terrain.track_options_id_dict["leap"]] > 0) \
                        & (-engaging_obstacle_info[:, 1 + self.terrain.max_track_options + 1] < engaging_obstacle_info[
                                                                                                :, 0]) \
                        & (engaging_obstacle_info[:,
                           0] < 0.)  # engaging jump-up, not engaging jump-down, positive distance.
        ...
        world_vel_error = torch.sum(torch.square(self.commands[:, :2] - self.root_states[:, 7:9]), dim=1)
        return (1 - torch.exp(
            -world_vel_error / self.cfg.rewards.tracking_sigma)) * engaging_mask  # reverse version of tracking reward
