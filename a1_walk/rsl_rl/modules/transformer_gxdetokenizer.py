...
class ActionDetokenizer(torch.nn.Module):
    def __init__(self, name, embedding_dim, action_dim, global_input=False, device='cuda'):
        ...
        self.detokenizers = nn.ModuleList(
            [nn.Linear(self.embedding_dim, 1) for _ in range(6)]
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.action_dim):
            if 0 <=i <3:
                curr_act = self.detokenizers[0](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
            elif i==3:
                curr_act = self.detokenizers[1](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
            elif i==4:
                curr_act = self.detokenizers[2](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
            elif 5 <=i <8:
                curr_act = self.detokenizers[3](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
            elif i==8:
                curr_act = self.detokenizers[4](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
            elif i==9:
                curr_act = self.detokenizers[5](x[:,i,:])
                action[:, i] = curr_act.squeeze(-1).float()  # 使用不同的线性层
        # print(action.shape)
        return action
...
class ValueDetokenizer(torch.nn.Module):
    def __init__(self, name, embedding_dim, global_input=False, device='cuda', nbodies = 14, act_dim = 12):
        super(ValueDetokenizer, self).__init__()

        self.nbodies = nbodies-1
        ...
        # 定义不同类型输入的linear embedding
        self.detokenizers = nn.ModuleList(
            [nn.Linear(self.embedding_dim, 1) for _ in range(8)]
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.num_actions+2):
            if 0 <=i <3:
                curr_val = self.detokenizers[0](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i==3:
                curr_val = self.detokenizers[1](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i==4:
                curr_val = self.detokenizers[2](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif 5 <=i <8:
                curr_val = self.detokenizers[3](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i==8:
                curr_val = self.detokenizers[4](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i==9:
                curr_val = self.detokenizers[5](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i ==10:
                curr_val = self.detokenizers[6](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
            elif i ==11:
                curr_val = self.detokenizers[7](x[:,i,:])
                values[:, i] = curr_val.squeeze(-1).float()  # 使用不同的线性层
...
# class ValueDetokenizer(torch.nn.Module):
#     def __init__(self, name, embedding_dim, global_input=False, device='cuda', act_dim = 12):
#         super(ValueDetokenizer, self).__init__()
...
class BodyCritic(nn.Module):
    def __init__(self, mapping, net, action_dim, embedding_dim, stack_time=True, global_input=False, device='cuda', nbodies = 13):
        ...
        self.net = net
        self.detokenizer = ValueDetokenizer(mapping, net.output_dim, global_input, device=device, act_dim =  action_dim)
