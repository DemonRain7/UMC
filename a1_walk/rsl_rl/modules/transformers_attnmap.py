...
from .nnTrf import TransformerEncoder,TransformerEncoderLayer
...
class ActorObsTokenizer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x_nodes, x_root.unsqueeze(1), x_isdoflimit.unsqueeze(1)), dim=1)
...
class Transformer(torch.nn.Module):
    def __init__(self, input_dim, dim_feedforward=128, nhead=6, nlayers=3, mask_position=None, numbodies = 14):
        ...
        self.encoder_layer = torch.nn.TransformerEncoderLayer(d_model=input_dim, nhead=nhead, dim_feedforward=dim_feedforward, batch_first=True, dropout=0.)
        self.encoder = torch.nn.TransformerEncoder(self.encoder_layer, num_layers=nlayers)
    ...
    def forward(self, x, mask = None):
        ...
        # 调试注意力权重
        # self._debug_attention_weights()

        return x


    def _debug_attention_weights(self):
        # 遍历 Transformer 层，调试每层注意力权重
        for i, layer in enumerate(self.encoder.layers):
            print(f"Debugging layer {i}")
            attn_layer = layer.attn_weight
            if attn_layer is None:
                print("self.attn_weight is None")
            else:
                print(attn_layer)
                print("self.attn_weight is not None")
