...
class BarrierTrack:
    ...
    def initialize_track(self):
        ...
        print("Seed when creating terrain:",random_state[1][0])  # 打印当前种子
        # print("track options:",len(self.track_kwargs["options"]),self.track_kwargs["options"])
    ...
    def get_starting_track(self, wall_thickness):
        # print("Entering get_starting_track")
    ...
    def get_engaging_block_info(self, base_positions, volume_points_offset):
        ...
        track_idx = self.get_track_idx(base_positions, clipped= False).long()
        track_idx_clipped = self.get_track_idx(base_positions).long() # (n, 2)
        ...
        block_idx = torch.floor(forward_distance / self.env_block_length).long() # (n,)
        block_idx_clipped = torch.clip(
            block_idx,
            0.,
            (self.n_blocks_per_track - 1),
