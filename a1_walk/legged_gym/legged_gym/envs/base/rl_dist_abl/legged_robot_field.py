...
class LeggedRobotField(LeggedRobot):
    ...
    def save_obs_to_csv(self):
        """ 将每次的obs_buf保存到CSV文件中 """
        if self.obs_buf is not None:
            # 假设 self.obs_buf 是 (num_envs, obs_dim) 的张量
            obs_buf_to_save = torch.cat([self.obs_buf[:, :48], self.obs_buf[:, -15:-12]], dim=1)
            obs_numpy = obs_buf_to_save.cpu().numpy()  # 转换为numpy数组
            df = pd.DataFrame(obs_numpy)  # 转换为 DataFrame
            
            # 追加写入数据到 CSV 文件
            
            obs_file_path = './legged_gym/logs/rollback_obs/obs_buffer.csv'
            df.to_csv(obs_file_path, mode='a', header=False, index=False)
    ...
    def compute_observations(self):
        ...
        # print(self.obs_buf.shape, self.obs_buf[:, -3:])
        ...
        # print(self.privileged_obs_buf.shape,self.privileged_obs_buf[:, -3:])
